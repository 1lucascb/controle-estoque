...
const state = { products: [], users: [], logs: [] };
...
function renderProductRows(products = state.products) {
    ...
    body.innerHTML = products.length ? products.map(product => `<tr><td><strong>${escapeHtml(product.name)}</strong><br><small>${escapeHtml(product.category || 'Sem categoria')}</small></td><td>${product.current_amount}</td><td>${product.min_stock_threshold}</td><td>${productStatus(product)}</td><td><div class="actions"><button class="icon-btn" title="Ajustar estoque" data-stock="${product.id}">${icon('package-plus')}</button><button class="icon-btn" title="Editar produto" data-edit-product="${product.id}">${icon('pencil')}</button><button class="icon-btn" title="Excluir produto" data-delete-product="${product.id}">${icon('trash-2')}</button></div></td></tr>`).join('') : '<tr><td colspan="5"><div class="empty-state">Nenhum produto encontrado.</div></td></tr>';
    refreshIcons();
}
function fillProductForm(product = {}) { const form = $('#product-form'); form.reset(); form.elements.productId.value = product.id || ''; form.elements.name.value = product.name || ''; form.elements.description.value = product.description || ''; form.elements.category.value = product.category || ''; form.elements.current_amount.value = product.current_amount ?? 0; form.elements.min_stock_threshold.value = product.min_stock_threshold ?? 5; form.elements.image_path.value = product.image_path || ''; }
async function initProducts() {
    setLoading($('#product-table-body')); try { await loadProducts(); renderProductRows(); } catch (error) { $('#product-table-body').innerHTML = `<tr><td colspan="5"><div class="empty-state">${escapeHtml(error.message)}</div></td></tr>`; }
    ...
    $('#product-form').addEventListener('submit', async event => { event.preventDefault(); const form = event.currentTarget; const id = form.elements.productId.value; const payload = { name: form.elements.name.value, description: form.elements.description.value || null, category: form.elements.category.value || null, current_amount: Number(form.elements.current_amount.value), min_stock_threshold: Number(form.elements.min_stock_threshold.value), image_path: form.elements.image_path.value || null }; try { await request(id ? `/api/v1/products/${id}` : '/api/v1/products', { method: id ? 'PUT' : 'POST', body: JSON.stringify(payload) }); closeModal('product-modal'); await loadProducts(); renderProductRows(); showToast(id ? 'Produto atualizado.' : 'Produto criado.'); } catch (error) { showToast(error.message, 'error'); } });
    ...
    $('#product-search').addEventListener('input', event => { const term = event.target.value.toLowerCase(); renderProductRows(state.products.filter(product => `${product.name} ${product.category || ''}`.toLowerCase().includes(term))); });
}
